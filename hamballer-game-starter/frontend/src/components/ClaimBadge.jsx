...
const ClaimBadge = () => {
      ...
      if (contracts?.xpVerifier && (badge.requiresProof || badge.xpEarned >= 50)) {
        try {
          ...
          if (proofError.message.includes('nullifier')) {
            ...
            showInvalidProof(`Invalid proof: Retry with updated XP`);
            setClaiming(prev => ({ ...prev, [badge.id]: false }));
            return;
  ...
  const retryBadgeClaim = async (badge) => {
    ...
    const retryCount = badge.retryCount || 0;
                    ...
                    <div className="badge-actions flex-shrink-0">
                      {canRetry ? (
                        <button
                          ...
                          className="mobile-button-sm bg-yellow-500 hover:bg-yellow-600 disabled:bg-yellow-500/50 text-black rounded text-sm transition-colors mobile-focus relative"
                        >
                          {retrying[badge.id] ? (
                            <div className="flex items-center space-x-2">
